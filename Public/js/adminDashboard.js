$(document).ready(function () {
  ...
  $selectFilter.change(function (event) {
    ...
    if (!arrayFilter.includes($etat)) return;
    ...
    if (requestPayments.length === 0) {
      ...
      if ($etat === "pending") renderPending(requestPayments);
      else
        renderAcceptedAndDeclined(
          requestPayments,
  ...
  $chercher.on("keypress keyup", function (event) {
    if ($selectFilter.val() === "pending") {
      const filtredRequests = getAllRequestsPayments().filter((req) =>
        req.nom_formateur.includes($(this).val().toUpperCase())
      );
      renderPending(filtredRequests);
    } else {
      const filtredRequests = getAllRequestsPayments(
        $selectFilter.val()
      ).filter((req) =>
        req.nom_formateur.includes($(this).val().toUpperCase())
      );
