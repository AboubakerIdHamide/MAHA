$(document).ready(function() {
    ...
    $deleteBtn.click(function() {
        ...
        const $titleFormation = $(this).parent().parent().parent().find(".card-title").text();
    ...
    const fillEditModal = (formation)=>{
        const formationInfo = [formation.find(".title").text().trim(), formation.find(".description").text().trim(), formation.find(".miniature").attr("src"), formation.find(".prix").text().trim(), formation.find(".categorie").attr("id"), formation.find(".langue").attr("id"), formation.find(".niveau").attr("id"), ];

        const inputsModal = [$("input#title"), $("#description"), $("#miniature"), $("#prix"), $("#categorie"), $("#langue"), $("#niveau"), ];
    ...
    $appliquerBtn.click(function(event) {
        ...
        if (ValidationInputs(modalInputs)) {
            ...
            formation.find(".categorie").attr("id", formationValues.categorie).text($("#categorie").find(`[value="${formationValues.categorie}"]`).text());
            formation.find(".langue").attr("id", formationValues.langue).text($("#langue").find(`[value="${formationValues.langue}"]`).text());
            formation.find(".niveau").attr("id", formationValues.niveauFormation).text($("#niveau").find(`[value="${formationValues.niveauFormation}"]`).text());
    ...
    const insertVideosIntoDOM = (videos)=>{
        ...
        if (typeof videos !== "object") {
            $(".accordion").html(`<div class="alert alert-danger" role="alert">${videos}</div>
      `);
            $("#videosModal").find(".modal-title").html("Videos");
            return null;
        }
        $("#videosModal").find(".modal-title").html("Videos - <span class='text-muted'>" + videos[0].nom_formation + "</span>");
      ...
      `);
            ...
            $editVideo.click(function() {
                ...
                if (videoTitle !== title.inputValue || videoDescription !== desc.inputValue) {
