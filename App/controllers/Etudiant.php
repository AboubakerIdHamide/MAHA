...
class Etudiant extends Controller
{
	...
	{
		// preparing data
		$data=$this->inscriptionModel->getInscriptionByEtudiant($_SESSION['user_id']);
	...
	public function coursVideos($idFormateur="", $idFormation=""){
		...
		// preparing data
		$data=$this->inscriptionModel->getInscriptionOfOneFormation($idFormation, $_SESSION['user_id'], $idFormateur);
	...
	{
		// Preparing Data
		$data=$this->videoModel->getWatchedVideos($_SESSION['user_id']);
	...
	{
        // Preparing Data
		$data=$this->videoModel->getBookmarkedVideos($_SESSION['user_id']);
		foreach($data as $video){
			// settingUp Video Link
			$video->url_video=$this->pcloudFile()->getLink($video->url_video);
		}
