$(document).ready(function () {
  ...
  $("#request-payment").click(function () {
    ...
    if (montant.length === 0) {
      ...
      if (montant.match(/^(?!0\d)\d*(\.\d+)?$/gm)) {
        ...
        showFlashMessage(
          "Format Incorrect, le montant doit contenaire que des chiffres.",
          "danger"
        );
      }
    }
  ...
  function getHistory() {
    const paymentHistory = $.parseJSON(
      $.ajax({
    ...
    if (typeof paymentHistory !== "object") return [];
    ...
    function showCloseBtn(state, idRequest) {
      if (state != "pending") {
        return (
          '<button id="' +
          idRequest +
          '" type="button" class="btn-close removeReq" aria-label="Close"></button>'
        );
      }
      return "";
    }
                ...
                <div class="col">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            ${showCloseBtn(
                              request.etat_request,
                              request.id_payment
                            )}
                        </div>
                        <div class="d-flex justify-content-between align-items-center card-body">
                            <span class="card-title">Montant <strong class="badge bg-${getBadgeColor(
                              request.etat_request
                            )} text-dark" style="font-size: 14px;">${
              request.request_prix
            } $</strong></span>
                            <span class="badge bg-${getBadgeColor(
                              request.etat_request
                            )} text-dark" style="font-size: 14px;">${
              request.etat_request
            }</span>
